...
static bool right_mouse_active(device_slot_t *slot, usb_mouse_report_t *mouse) {
    ...
    if (dx != 0 || dy != 0) {
        ...
        slot->mouse_step_x = (slot->smooth_mouse_x * 2.4) / 10.0;
        slot->mouse_step_y = (slot->smooth_mouse_y * 2.4) / 10.0;
        slot->mouse_steps_left = 10;
        slot->optical_stick_x = clamp_i16_to_i8((int)llround(slot->smooth_mouse_x * 14.0));
        slot->optical_stick_y = clamp_i16_to_i8((int)llround(slot->smooth_mouse_y * 14.0));
    }
